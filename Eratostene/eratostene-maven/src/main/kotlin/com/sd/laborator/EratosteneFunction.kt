...
class EratosteneFunction : FunctionInitializer(),
    ...
    override fun apply(msg : EratosteneRequest) : EratosteneResponse{// preluare numar din parametrul de intrare al functiei
        val number = msg.getAnything()
        val response = EratosteneResponse()
        //// se verifica daca numarul nu depaseste maximul
        //if (number >= eratosteneSieveService.MAX_SIZE) {
        //    LOG.error("Parametru prea mare! $number > maximulde${eratosteneSieveService.MAX_SIZE}")
        //    response.setMessage("Se accepta doar parametri maimicica" + eratosteneSieveService.MAX_SIZE)
        //    return response
        //}
        LOG.info("Se calculeaza primele $number numere prime ...")// se face calculul si se seteaza proprietatile pe obiectulcurezultatul
...
/**
 * This main method allows running the function as a CLI applicationusing: echo '{}' | java -jar function.jar
